...
library(gridExtra)
  ...
  load("./Output/CEVT_Results_Saved_Latest.RData", verbose = TRUE)
  ...
  tabPanel("Current year",
           ...
           sidebarLayout(
             sidebarPanel(width = 2,
                          # sliderInput("slideryr1", "Choose year",  min = min(yr_rng), max = max(yr_rng), value = max(yr_rng),
                          #             width = "400px", ticks = TRUE, sep = "", step = 1),
                          # sliderInput("sliderday1", "Choose day of year",  min = 10, max = 360, value = current_day,
                          #             width = "400px", ticks = TRUE, sep = "", step = 10),
                          br(),
                          checkboxInput("add_forest1", "Add 2nd model", value = FALSE),
                          checkboxInput("add_TAC1", "Add TAC to plot", value = TRUE),
                          numericInput("tac1", "Total allowable catch (mt):", 1600, min = 0, max = 2000, step = 100, width = "200px")
             ),
             
             mainPanel(
               br(),
               plotOutput("Pred_plot_last"),
               br(),
               br(),
               br(),
               br(),
               br(),
               plotOutput("Vesbar_last"),
             ))
  ),
...
server <- function(input, output) {
  ...
  output$Pred_plot_last <- renderPlot({
